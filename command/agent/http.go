...
// setLastToken is used to set the next token header for pagination
func setLastToken(resp http.ResponseWriter, nextToken string) {
	if nextToken != "" {
		resp.Header().Set("X-Nomad-LastToken", nextToken)
	}
}
...
func parsePagination(req *http.Request, b *structs.QueryOptions) {
	...
	b.NextToken = query.Get("next_token") // DEPRECATED
	b.LastToken = query.Get("last_token")
}
